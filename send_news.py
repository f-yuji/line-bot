...
STRONG_KEYWORDS: Dict[str, List[str]] = {
    "real_estate": [
        ...
        "不動産バブル", "土地取引", "オフィス空室", "物流不動産", "データセンター用地",
        "インバウンド不動産", "外国人購入", "相続土地", "空き家問題", "タワマン",
        "賃料上昇", "地代上昇", "不動産クラウド", "不動産ファンド", "私募REIT",
    ],
    ...
    "interest_rates": [
        ...
        "FOMC", "パウエル議長", "テーパリング", "量的緩和", "量的引き締め",
        "フォワードガイダンス", "ドット図", "中立金利", "ターミナルレート",
        "逆イールド", "スプレッド拡大", "信用スプレッド", "日銀利上げ", "利上げペース",
        "インフレ指標", "CPIショック", "PCEデフレーター", "賃金インフレ",
    ],
...
def summarize_articles(news_list: List[Dict[str, str]]) -> tuple:
    ...
    if uncached:
        ...
        prompt = (
            ...
            "【chat】投資判断コメント（1〜2文、20〜50文字以内）\n"
            "個人投資家向けに、このニュースをどう受け取り・どう動くべきか方向感を示す。\n"
            "必ず以下のいずれかのスタンスを明示する:\n"
            "・買い圧力 / 売り圧力 / 様子見 / 注目 のどれかを文中に自然に入れる\n"
            "・「〜のため〇〇株に買い圧力」「〜なら様子見が無難、理由は〜」「〜に注目」など\n"
            "・方向感が読めない場合は「材料出尽くし感」「判断保留」と正直に書く\n"
            "禁止: 事実の言い換え／感想文（「気になる」等）／情報ゼロの文\n\n"
