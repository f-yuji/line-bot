"""
Rule-based rebound scoring.

The score is intentionally usable before the ML phases exist:
technical (50) + fundamental (30) + market (20), then external
adjustments such as bad-news penalties. The public return shape stays
...
def technical_score(closes, volumes, price_at_drop: float | None, cfg: dict) -> float:
    ...
    try:
        ...
        if prev > 0:
            ...
            thr = float(cfg.get("daily_rebound_threshold", 4.0))
        ...
        if price_at_drop and price_at_drop > 0:
            ...
            thr = float(cfg.get("drop_rebound_threshold", 8.0))
        ...
        if len(volumes) >= 21:
            ...
            thr = float(cfg.get("volume_ratio_threshold", 2.0))
        ...
        if len(rsi_s) >= 6:
            ...
            rsi_rec = float(cfg.get("rsi_recover_threshold", 40.0))
            rsi_low = float(cfg.get("rsi_low_threshold", 25.0))
...
def fundamental_score(
    ...
    """Fundamental score, capped at 30. Deficit companies are fixed at 0."""
    if is_deficit:
        return 0.0
...
def market_score(nikkei_pct: float | None) -> float:
    """Market score. Broad selloffs score higher for rebound setups."""
...
def score_label(total: float, cfg: dict) -> str:
    if total >= float(cfg.get("strong_watch_score", 85.0)):
        return "強監視★★"
    if total >= float(cfg.get("watch_score", 75.0)):
        return "監視"
    if total >= float(cfg.get("ignore_score", 65.0)):
        return "観察"
    return "スルー"
...
def calculate_score(
    ...
    *,
    ...
    total = (
        t + f + m
        - float(adj.get("bad_news_penalty") or 0)
        - float(adj.get("sector_risk_penalty") or 0)
        + float(adj.get("shock_bonus") or 0)
        + float(adj.get("fx_bonus") or 0)
    )
